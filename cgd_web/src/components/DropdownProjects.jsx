...
function DropdownProjects({ align, project_id }) {
  ...
  const fetchProjectsResult = async () => {
    try {
      ...
      if (projectsResult.detail && projectsResult.detail === "Token expired")
        nav("/login");
    } catch (error) {
      console.log(error);
    }
  ...
  const ProjectListItem = projects.map((project) =>
    project.id == project_id ? null : (
      <li key={project.id} className="pt-0.5 pb-2 px-3 mb-1">
        <Link
